...
class VideosDAOImpl(private val bd: JdbcTemplate):VideosDAO {
    ...
    //override fun chercherTous(): List<Video> = bd.query("select id, prenom from Joueurs") { réponse, _ ->
        //Video(id_video, titre, description, miniature, fichiervideo, status, auteur, datePublication)(réponse.getString("id"), réponse.getString("prenom"))
    //}

    //override fun chercherParId(id: Int): Video? = bd.query("select id, prenom from Joueurs where id = ?", id) { réponse, _ ->
    //    Joueur(réponse.getString("id"), réponse.getString("prenom"))
    //}.singleOrNull)
